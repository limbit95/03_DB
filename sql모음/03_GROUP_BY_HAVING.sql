...
-- 여러 개의 결과값을 산출하기 위해 그룹 함수가 적용된 그룹의 기준을 ORDER BY절에 기술하여 사용
...
-- EMPLOYEE 테이블에서
-- 직급코드가 같은 같은 사람의 직급코드, 급여 평균, 인원 수를
-- 직급코드 오름차순으로 조회
...
SELECT DECODE(SUBSTR(EMP_NO, 8, 1), '1', '남', '2', '여') "성별", 
			 ...
			 SUM(SALARY) "급여 합"
FROM EMPLOYEE
GROUP BY DECODE(SUBSTR(EMP_NO, 8, 1), '1', '남', '2', '여') -- 별칭 사용 X (별칭 사용한 SELECT 절 보다 GROUP BY 절 먼저 해석하기 때문에)
ORDER BY "인원 수";
...
SELECT DEPT_CODE, JOB_CODE, COUNT(*) "사원수"
FROM EMPLOYEE
GROUP BY DEPT_CODE, JOB_CODE
-- DEPT_CODE 로 그룹을 나누고, 나눠진 그룹내에서
-- JOB_CODE 로 그룹을 또 분류 (세분화)
...
--UNION -- 12행 : 유재식, 정중하, 박나라, 하이유, 김해술, 심봉선, 윤은해, 대북혼, 전지연
--INTERSECT -- 2행 : 심봉선, 대북혼
--UNION ALL --  14행 : 중복값인 심봉선과 대북혼 중복값 그대로 출력
MINUS -- 위의 SELECT문의 결과값에서 아래의 SELECT문의 결과값을 빼고 출력
			-- 즉, 위와 아래의 교집합을 뺀 위의 SELECT문 결과값 출력
