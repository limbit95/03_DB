...
-- ROLE_SYS_PRIVS : ORACLE DB에서 시스템 권한을 가진 역할을 나타내는 데이터 딕셔너리 뷰
WHERE ROLE = 'RESOURCE';
-- CREATE 트리거, 시퀀스, 테이블 등 총 8가지 권한이 부여되어있음
...
ALTER USER lsh_sample DEFAULT TABLESPACE 
...
-- kh_lsh 	/		lsh_sample 사용자 계정끼리 서로 객제 접근 권한 부여
...
-- 2. (kh_lsh) sample 계정에 EMPLOYEE 테이블 조회 권한 부여 
...
-- 4. (kh_lsh) sample 계정에 EMPLOYEE 테이블 조회 권한 회수 (REVOKE)

-- [권한 회수 작성법]
-- REVOKE 객체권한 ON 객체명 FROM 사용자명;
REVOKE SELECT ON EMPLOYEE FROM lsh_sample;

-- 5. (lsh_sample) 권한 회수 확인
SELECT * FROM kh_lsh.EMPLOYEE;
-- ORA-00942: 테이블 또는 뷰가 존재하지 않습니다
