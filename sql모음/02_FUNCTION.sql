...
SELECT INSTR('AABAACAABBAA', 'B') FROM DUAL; -- 3번째 위치

-- 문자열을 5번째 문자부터 검색하여 첫 번째 B 위치를 조회
SELECT INSTR('AABAACAABBAA', 'B', 5) FROM DUAL; -- 9번째 위치

-- 문자열을 5번째 문자부터 검색하여 두 번째 B 위치를 조회
SELECT INSTR('AABAACAABBAA', 'B', 5, 2) FROM DUAL; -- 10번째 위치
...
SELECT ROUND(123.456, -1) FROM DUAL; -- 120 -- 소수점 0번째 자리에서 반올림
																		 -- 소수점 -1 자리 표현
																		 -- == 1의 자리에서 반올림해서 10의 자리부터 표현

SELECT ROUND(123.456, -2) FROM DUAL; -- 100 -- 10의 자리에서 반올림해서 100의 자리부터 표현
...
-- 2024-02-29 11:20:44.289 +0900 (UTC에서 +9시간 나타냄, 한국 표준시)

-- MONTHS_BETWEEN(날짜, 날짜) : 두 날짜의 개월 수 차이 반환
SELECT ROUND( MONTHS_BETWEEN(SYSDATE, '2024-06-26'), 3) FROM DUAL;
...
-- 0 : 숫자 한 칸을 의미, 여러개 작성 시 오른쪽 정렬 + 빈 칸 0 추가
-- L : 현재 DB에 설정된 나라의 화폐 기호

SELECT '숫자 -> 문자열 : ' || TO_CHAR(1234), '숫자 : ' || 1234 FROM DUAL;
...
SELECT SYSDATE FROM DUAL; -- 2024-02-29 12:20:57.000
...
-- COUNT() : 행 개수를 헤아려서 반환
-- COUNT([DISTINCT] 컬럼명) : 중복을 제거한 행 개수를 헤아려서 반환
-- COUNT(*) : NULL을 포함한 전체 행 개수를 반환
-- COUNT(컬럼명) : NULL을 제외한 실제 값이 기록된 행 개수를 반환
