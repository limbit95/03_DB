...
--				인덱스(INDEX), 사용자(USER), 패키지(PACKAGE),
--				트리거(TRIGGER), 프로시져(PROCEDURE), 함수(FUNCTION),
--				동의어(SYNONYM)...
...
CHAR(크기) : 고정길이 문자형 (최대 2,000 BYTE)
	-> CHAR(10) 컬럼에 'ABC' 3BYTE 문자열만 저장해도 10BYTE 저장공만 모두 사용
	
VARCHAR2(크기) : 가변길이 문자형 (최대 4,000 BYTE)
...
CREATE TABLE "MEMBER"(
	...
	MEMBER_SSN CHAR(14), -- 1213-1234567
	ENROLL_DATE DATE DEFAULT SYSDATE
);

-- SQL 작성법 : 대문자 작성 권장, 연결된 단어 사이에는 "_" 사용
-- 문자인코딩 UTF-8 : 영어, 숫자 1BYTE, 한글, 특스문자 등은 3BYTE 취급
...
-- INSERT INTO MEMBER VALUES(값1, 값2...)
...
INSERT INTO "MEMBER" VALUES
('MEM04', 'PASS04', '김길동', '990303-1333333',
...
DESC USER_CONTRAINTS; -- SQLPLUS에서만 됨 디비버에서는 지원X
...
-- NULL을 ("KH_LSH"."USER_USED_NN"."USER_NO") 안에 삽입할 수 없습니다
...
-- 던 ,UNIQUE 제약조건이 설정된 컬럼에 NULL 값은 중복 삽입 가능
...
INSERT INTO USER_USED_UK
VALUES(1, 'USER01', 'PASS01', '홍길동', '남', '010-1234-5678', 'hong123@kh.or.kr');
-- ORA-00001: 무결성 제약 조건(KH_LSH.USER_ID_U)에 위배됩니다
...
-- ORA-00001: 무결성 제약 조건(KH_LSH.USER_ID_NAME_U)에 위배됩니다
...
-- 기본키 중복 오류
-- ORA-00001: 무결성 제약 조건(KH_LSH.USER_NO_PK)에 위배됩니다

INSERT INTO USER_USED_PK
VALUES(NULL, 'USER03', 'PASS03', '유관순', '여', '010-9999-5678', 'yoo123@kh.or.kr');
-- 기본키 NULL 이므로 오류
-- ORA-01400: NULL을 ("KH_LSH"."USER_USED_PK"."USER_NO") 안에 삽입할 수 없습니다
...
INSERT INTO USER_USED_PK2
VALUES(1, 'USER01', 'PASS02', '신사임당', '남', '010-1234-5678', 'hong123@kh.or.kr');
-- ORA-00001: 무결성 제약 조건(KH_LSH.PK_USERNO_USERID)에 위배됩니다
-- 회원번호와 아이디 둘 다 중복되었을 때만 제약조건 위배 에러 발생

INSERT INTO USER_USED_PK2
VALUES(NULL, 'USER01', 'PASS02', '신사임당', '남', '010-1234-5678', 'hong123@kh.or.kr');
-- ORA-01400: NULL을 ("KH_LSH"."USER_USED_PK2"."USER_NO") 안에 삽입할 수 없습니다
...
-- 참조할 테이블의 참조할 컬럼명이 생략되면, PRIMARY KEY로 설정된컬럼이 자동 참도할 컬럼이 됨
...
-- ORA-02291: 무결성 제약조건(KH_LSH.GRADE_CODE_FK)이 위배되었습니다- 부모 키가 없습니다
...
-- ORA-02292: 무결성 제약조건(KH_LSH.GRADE_CODE_FK)이 위배되었습니다- 자식 레코드가 발견되었습니다
...
-- ORA-02290: 체크 제약조건(KH_LSH.GENDER_CHECK)이 위배되었습니다

---------------------------------------------------------------------------------------------

-- 8. SUBQUERY를 이용한 테이블 생성
-- 컬럼명, 데이터 타입, 값이 복사되고, 제약조건은 NOT NULLA만 복사됨
...
-- 서브쿼리로 테이블 생성 시
-- 테이블의 형태(컬럼명, 데이터 타입) + NOT NULL 제약 조건만 복사됨
-- 이외 다른 제약조건, 코멘트는 복사 되지 않기 때문에 별도 추가 작업이 필요
...
-- -> 참조 테이블의 PK를(기본키)를 FK로 사용하는 경우 참조 컬럼명 생략 가능

-- ALTER TABLE 테이블명 MODIFY 컬럼명 NOT NULL;

-- NOT NULL 제약조건만 복사된 EMPLOYEE_COPY 테이블에
-- ENP_ID 컬럼에 PRIMARY KEY 제약조건 추가
...
-- *** 수업 시간에 활용하던 테이블에는 FK 제약조건이 없는 상태이므로로 추가해보자
...
-- 참조 테이블은 SAL_GRADE, 참조 컬럼명 SAL_GRADE의 기본키
