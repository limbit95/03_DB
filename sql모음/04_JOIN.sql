...
-- 1. 내부 조인(INNER JOIN) (ORACLE에서는 등가 조인(EQUAL JOIN)이라 부름)
-- 		연결되는 컬럼의 값이 일치하는 행들만 조인됨
--    (== 일치하는 값이 없는 행은 조인에서 제외됨)

-- 작성 방법은 크게 ANSI 구문과 ORACLE 구문으로 나뉘고
...
--						-> 같은 이름으로 누구의 컬럼명을 지칭하는지 애매하기에 별칭을 지어 지칭해야 함
...
-- 사용할 때 반드시 OUTER JOIN 임을 명시해야 한다
...
-- 												모든 행을 결과에 포함
-- ** ORACLE 에서는 FULL OUTER JOIN 사용 못함 **
